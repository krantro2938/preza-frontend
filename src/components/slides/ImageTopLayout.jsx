...
export default function ImageTopLayout({ slide, slideNumber, theme }) {
  const renderMarkdown = (text) => {
    ...
    return text
      ...
      .replace(/- (.*?)(?=\n|$)/g, (match, content) => {
        bulletCount++;
        return `<div class="flex items-start mb-4"><span class="${theme.content.bullet} font-bold mr-3 text-md">${bulletCount.toString().padStart(2, '0')}.</span><span class="${theme.content.text} text-md">${content}</span></div>`;
      })
      .replace(/\n/g, '');
  };
    ...
    <div className={`relative w-full h-full ${theme.content.bg} overflow-hidden`}>
      ...
      <div className="relative z-20 -mt-8" style={{ height: '57.33%', padding: '5.07% 7.5% 0' }}>
        <div className="text-center max-w-5xl mx-auto">
          ...
          <div className={`h-1 ${theme.content.line} rounded-full mx-auto mb-6`} style={{ width: '90px' }}></div>
